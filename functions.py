...
def get_nuclides_dataframe():
    ...
    return nuclides_dataframe

def get_dc_hl():
    nuclide_value = st.session_state.nuclide
    nuclides_dataframe = get_nuclides_dataframe()
    selected_nuclide=nuclides_dataframe[nuclides_dataframe['name']==nuclide_value]
    half_life=np.array(selected_nuclide['halfLife'])[0]
    decay_constant=np.array(selected_nuclide['decayConstant'])[0]
